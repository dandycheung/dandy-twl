...
{
public:
	static const TUint* GetCrcTable()
	{
	...
	static const TUint32* GetCrc32Table()
	{
		static const TUint32 crc32Table[256] =
		{
	...
	//
	static void Calculate(TUint16& aCrc, const void* aPtr, TInt aLength)
	{
		...
		TUint crc = aCrc;
		while(pB < pE)
			crc = (crc << 8) ^ GetCrcTable()[((crc >> 8) ^ *pB++) & 0xff];

		aCrc = (TUint16)crc;
	}
	
	
	static void Calculate(TUint32& aCrc, void* aPtr, TInt aLength)
	{
		...
		{
			byte = p[index];
			temp = (aCrc ^ byte) & 0x000000FF;
			aCrc = ((aCrc >> 8) & 0x00FFFFFF) ^ GetCrc32Table()[temp];
			index++;
		}
	}
};
...
protected:
	TUint Check() const
	{
		...
		*pT++ = iType[0].iUid >> 8 & 0xff;
		*pT++ = iType[0].iUid >> 24 & 0xff;
		*pT++ = iType[1].iUid >> 8 & 0xff;
		*pT++ = iType[1].iUid >> 24 & 0xff;
		*pT++ = iType[2].iUid >> 8 & 0xff;
		*pT++ = iType[2].iUid >> 24 & 0xff;

		TUint16 crc = 0;
		Crc::Calculate(crc, &buf[0], (KMaxCheckedUid * sizeof(TUid)) >> 1);
		const int hi = crc;

		/* lo */
		pT = &buf[0];

		*pT++ = iType[0].iUid >> 0 & 0xff;
		*pT++ = iType[0].iUid >> 16 & 0xff;
		*pT++ = iType[1].iUid >> 0 & 0xff;
		*pT++ = iType[1].iUid >> 16 & 0xff;
		*pT++ = iType[2].iUid >> 0 & 0xff;
		*pT++ = iType[2].iUid >> 16 & 0xff;
...
{
public:
	TCheckedUidX(TUint uids[3])
		: TCheckedUid(TUidType(TUid::Uid(uids[0]), TUid::Uid(uids[1]), TUid::Uid(uids[2]))) 
	{
	}
	...
	/*
		TUint uids[4];
		/// TODO: uids[0] = a; uids[1] = b; uids[2] = c; ...
